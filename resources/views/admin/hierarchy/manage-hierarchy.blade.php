    ...
    <div class="space-y-6">
            ...
            @else
                <div class="rounded-[2rem] bg-white p-6 shadow-xl shadow-slate-900/5">
                    ...
                    @if(auth()->user()->canAccessAdminPanel() && auth()->user()->hasPermissionTo('progress.view'))
                ...
                <div class="rounded-[2rem] bg-white p-6 shadow-xl shadow-slate-900/5">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        ...
                        <form method="GET" action="{{ route('hierarchy.manage') }}" class="grid grid-cols-1 gap-3 md:grid-cols-5 w-full lg:w-auto">
                            <div>
                            ...
                            <div>
                                <label for="per_page" class="block text-xs font-medium uppercase tracking-wider text-slate-500">Rows</label>
                                <select name="per_page" id="per_page" class="mt-1 block w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                ...
                >
                                        ...
                                        <td class="px-6 py-4 align-top">
                                            <div>
                                                ...
                                                <div class="mt-3 flex flex-wrap gap-2">
                                                    <a href="{{ route('hierarchy.members.show', $snapshot['user']) }}" class="inline-flex items-center rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-100">
                                                        View record
                                                    </a>
                                                    @if(auth()->user()->canAccessAdminPanel() && auth()->user()->hasPermissionTo('progress.view'))
                                                        <a href="{{ route('admin.progress.user', $snapshot['user']) }}" class="inline-flex items-center rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-600 transition hover:border-slate-300 hover:text-slate-900">
                                                            Scoped report
                                                        </a>
                                                    @endif
                                                </div>
                    ...
                    @if(method_exists($memberSnapshots, 'links'))
                        <div class="border-t border-slate-200 px-6 py-4">
                            {{ $memberSnapshots->links() }}
                        </div>
                    @endif
                </div>
            @endif
