            ...
            <form method="POST" action="{{ route('admin.users.store') }}">
                ...
                <div class="mb-4">
                  ...
                  <select name="role" id="role" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>
                                        @foreach(\App\Models\User::distinct()->pluck('role') as $roleValue)
                                            <option value="{{ $roleValue }}" {{ old('role') === $roleValue ? 'selected' : '' }}>{{ ucfirst($roleValue) }}</option>
                                        @endforeach                  </select>                    @error('role')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
