...
class GenerateReadingPlan extends Command
{
    ...
    public function handle()
    {
        ...
        if ($readingPlan->dailyReadings()->exists()) {
            $readingPlan->dailyReadings()->delete();
        }

        $planDefaults = ReadingPlan::defaultsFor($readingPlan->type);
        $testament = $planDefaults['testament'];
        ...
        $chaptersPerDay = max($readingPlan->chapters_per_day, 1);
        $totalReadingDays = ceil($totalChapters / $chaptersPerDay);

        // Calculate total days including break days
        $streakDays = max($readingPlan->streak_days, 1);
        $breakDays = max($readingPlan->break_days, 0);
